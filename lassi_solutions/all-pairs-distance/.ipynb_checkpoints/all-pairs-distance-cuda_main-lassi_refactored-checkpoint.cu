...
/* coalesced GPU implementation of the all-pairs kernel using
   character data types, registers, and shared memory with a balanced tree reduction */
__global__ void k2 (const char *data, int *distance) {
  ...
  /* initialize shared memory per thread */
  dist[idx] = 0;
  __syncthreads();
...
int main(int argc, char **argv) {
  ...
  // Allocate pinned memory for arrays used in GPU transfers for improved transfer speeds and reduced overhead.
  cudaMallocHost((void **)&data_char, INSTANCES * ATTRIBUTES * sizeof(char));
  cpu_distance = (int *)malloc(INSTANCES * INSTANCES * sizeof(int));
  cudaMallocHost((void **)&gpu_distance, INSTANCES * INSTANCES * sizeof(int));
...
#pragma omp parallel for collapse(2)
  ...
  /* CPU execution */
  memset(cpu_distance, 0, INSTANCES*INSTANCES*sizeof(int));
  gettimeofday(&tp, &tzp);
  ...
  /* Launch GPU kernel k1 */
  elapsedTime = 0; 
  for (int n = 0; n < iterations; n++) {
    /* Zero the GPU distance array directly on the device */
  ...
  cudaMemcpy(gpu_distance, distance_device,
             INSTANCES * INSTANCES * sizeof(int), cudaMemcpyDeviceToHost);
  printf("Average kernel execution time (k2): %f (us)\n", elapsedTime / iterations);
  status = memcmp(cpu_distance, gpu_distance, INSTANCES * INSTANCES * sizeof(int));
  if (status != 0) printf("FAIL\n");
  else printf("PASS\n");

  /* Launch GPU kernel k3 */
  elapsedTime = 0; 
  for (int n = 0; n < iterations; n++) {
    cudaMemset(distance_device, 0, INSTANCES * INSTANCES * sizeof(int));
  ...
  cudaMemcpy(gpu_distance, distance_device,
             INSTANCES * INSTANCES * sizeof(int), cudaMemcpyDeviceToHost);
  printf("Average kernel execution time (k3): %f (us)\n", elapsedTime / iterations);
  status = memcmp(cpu_distance, gpu_distance, INSTANCES * INSTANCES * sizeof(int));
  if (status != 0) printf("FAIL\n");
  else printf("PASS\n");
  ...
  free(cpu_distance);
  free(data);
  cudaFreeHost(data_char);
  cudaFreeHost(gpu_distance);
